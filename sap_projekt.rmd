...
```{r}
nogometasi <- read_csv('dataset.csv')
...
najbolji_strijelci <- slice_max(nogometasi, Gls, n=5) %>% select(Player, Team, Gls, Gls.1, Ast)
najbolji_asistenti <- slice_max(nogometasi, Ast, n=5) %>% select(Player, Team, Gls, Ast, Ast.1)
```
