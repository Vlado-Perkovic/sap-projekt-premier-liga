...
nezavisnost provest ćemo ANOVA test: Nulta hipoteza: Raspodijela minuta
igračima do 25 godina se ne razlikuje po klubovima Alternativna
hipoteza: Raspodijela minuta igračima do 25 godina razlikuje se u barem
jednom klubu alpha = 0.05.
...
Ne odbacujemo nultu hipotezu da se raspodijela minuta razlikuje po
klubovima.
...
asistencija. Naravno nije objektivno usporedjivat obrambene vezne i
...
ggplot(dobri, aes(x = Ast, y = Gls)) + geom_jitter(width = 0.4, height = 0.4, alpha = 0.4) + geom_text(aes(label = Player), check_overlap = T, size = 2.5) + geom_jitter(data = losi, aes(x = Ast, y = Gls), width = 0.3, height = 0.3, alpha = 0.4)
```
...
ggplot(dobrixG, aes(x = xA, y = xG)) + geom_jitter(width = 0.3, height = 0.3, alpha = 0.4) + geom_text(aes(label = Player), check_overlap = T, size = 2.5) + geom_jitter(data = losixG,aes(x = xA, y = xG), width = 0.3, height = 0.3, alpha = 0.4 )
```
...
**Određivanje uspješnosti po broju asistencija preko mjere očekivanih
asistencija.**

Prikažimo raspodjelu na grafu...

```{r}
ggplot(nog, aes(x = xA, y = Ast)) + geom_point(alpha = 0.4) + stat_smooth(method = lm, formula = y~x, se = F)
```

Prema grafu se da naslutiti da postoji linearna veza između te dvije
mjere, no uvjerljivo kao golovi. Svejedno, provest ćemo istraživanje i
vidjeti rezultate.
...
Kao mjeru valjanosti linearne veze razmatramo varijablu R^2^. Ona iznosi
0.64 što nije dovoljno dobro kao prethodno istraživanje. Iako je R2
relativna mjera, smatramo da nije dovoljno dobra za reći da mjerom xA
možemo precizno odrediti uspješnost igrača.

Ovaj rezultat prirodno ima smisla kada pogledamo nogometni kontekst
očekivane asistencije koja ovisi o tome hoće li asistirani igrač zabiti.
...
```{r}
ggplot(nog, aes(x = npxG.xA.1, y = G.A.PK)) + geom_point() + stat_smooth(method = lm, formula = y~x, se = F)
```
...
homogenosti varijance raspodijele stranih i domaćih igrača u odnosu sa
...
stranih. Dobivena p-vrijednost je vrlo velika i to nam govori kako je
naša prvotna pretpostavka bila točna, a to
