...
ggplot(dobri, aes(x = Ast, y = Gls)) + geom_jitter(width = 0.4, height = 0.4) + geom_text(aes(label = Player), check_overlap = T, size = 2.5) + geom_jitter(data = losi, aes(x = Ast, y = Gls), width = 0.3, height = 0.3)
```
...
ggplot(dobrixG, aes(x = xA, y = xG)) + geom_jitter(width = 0.3, height = 0.3) + geom_text(aes(label = Player), check_overlap = T, size = 2.5) + geom_jitter(data = losixG,aes(x = xA, y = xG), width = 0.3, height = 0.3 )
```
...
```{r}
ggplot(nog, aes(x = xG, y = Gls)) + geom_point() + stat_smooth(method = lm, formula = y~x, se = F)
```
...
```{r}
ggplot(nog, aes(x = npxG.xA, y = G.A)) + geom_point() + stat_smooth(method = lm, formula = y~x, se = F)
```
